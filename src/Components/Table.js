...
function createData(beeName, locationOfBooking, dateBooked) {
  return {
    beeName, locationOfBooking, dateBooked,
  };
}
const rowData = filtered.map((reservation) => createData(
  reservation.name, reservation.city, reservation.date,
));

// const rows = [
//   createData(`${filtered[0].name}`, 159, 6.0, 24, 4.0),
      ...
      <Table sx={{ minWidth: 200 }} aria-label="simple table">
        <TableHead>
          <TableRow sx={{ backgroundColor: 'cadetblue' }}>
            ...
            <TableCell align="right">City Booked</TableCell>
            <TableCell align="right">Date of Booking</TableCell>
            <TableCell align="right">Keep Booking?</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {rowData.map((row) => (
